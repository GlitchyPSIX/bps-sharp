...
namespace BPS_Sharp
{
    static public class BPS
    {

        static public async Task TryBPS(string ROM, string Patch, string path)
        {
            await Task.Run(() => { File.WriteAllBytes(path, ApplyBPS(File.ReadAllBytes(ROM), File.ReadAllBytes(Patch))); });
        }

        static byte[] ApplyBPS(byte[] ROM, byte[] Patch)
        {
